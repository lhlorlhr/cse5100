...
run_final_evals() {
    ...
    if [[ "${RUN_FIXED_OPP_EVAL}" == "1" ]]; then
        echo "running fixed-opponent eval for exp=${exp_name}, seed=${seed}, comm_dim=${comm_dim}"
    ...
    if [[ "${RUN_CUTOFF_EVAL}" == "1" ]]; then
        echo "running cutoff-intervention eval for exp=${exp_name}, seed=${seed}, comm_dim=${comm_dim}"
...
for comm_dim in "${COMM_DIMS[@]}"; do
    for lambda_value in "${LAMBDA_VALUES[@]}"; do
        ...
        exp_name="step2_dim${comm_dim}_lambda${sanitized_lambda}"
        ...
        for seed in "${SEEDS[@]}"; do
            ...
            run_final_evals "${exp_name}" "${seed}" "${comm_dim}" "${latest_run_dir}"
        done
    done
done
